...
impl Fprom {
    ...
    pub fn get_read_address(&self, buffer_len: usize) -> usize {
        ...
        let mut first_non_empty_cell_address = self.address;
        ...
        while non_empty_cell_counter == 0 {
            ...
            if self.flash[cell_address] == 0xffff {
                if cell_address >= self.address + buffer_len {
                    first_non_empty_cell_address = cell_address - buffer_len * 2;
                } else {
                    first_non_empty_cell_address = 0;
                }
                non_empty_cell_counter = 1;
            } else {
                if cell_address >= self.address + self.pages_num * self.flash.get_page_size() {
                    non_empty_cell_counter = 1;
                    first_non_empty_cell_address = self.address + self.pages_num * self.flash.get_page_size() - buffer_len;
                } else {
                    cell_address += 2;
                }
            }
